...
def _(mo):
    ...
    First, we load an example signal data, which is a recording of a fin-whale
    song. The data is stored in a NumPy `.npz` file, and contains the time
    points, acoustic amplitude values, and the sampling frequency of the recording.

    We define a `Signal` dataclass to encapsulate the information above and
...
def _(mo):
    ...
    First, we need to create a filter bank with the desired parameters. After
...
def _(mo):
    ...
    We can also visualize the filter responses in the frequency domain by
    plotting the impulse responses of the filters. This can help us understand
    how each filter responds to a brief input signal and how it captures
    different frequency components of the signal.

    In the figure below, we also show the minimum and maximum frequencies of
    the filterbank, which correspond to the lowest and highest frequencies that
    the filters can capture. The maximum frequency is determined by the Nyquist
...
def _(filt_bank, plot_freq_resps, plt, signal):
    ...
    for f in (f_low, f_nyq):
        ...
        ax_resps.text(
            ...
            bbox={"boxstyle": "round", "fc": "white", "ec": "#cc0000", "alpha": 0.75},
        )
