...
class MorletWavelet(MorletWaveletGroup):
    ...
    def __init__(
        ...
        dtype : torch.dtype, default=torch.float64
            Data type for floating point tensors.
        learnable_center_freq : bool, default=False
            If True, center frequency becomes a learnable parameter.
        learnable_shape_ratio : bool, default=False
            If True, shape ratio becomes a learnable parameter.
    ...
    def __repr__(self) -> str:
        learnable_cf = isinstance(self._center_freqs, nn.Parameter)
        learnable_sr = isinstance(self._shape_ratios, nn.Parameter)
        learnable_str = ""
        if learnable_cf or learnable_sr:
            learnable_parts = []
            if learnable_cf:
                learnable_parts.append("Fc")
            if learnable_sr:
...
class MorletFilterBank(MorletWaveletGroup):
    ...
    def __init__(
        ...
        dtype : torch.dtype, default=torch.float64
            Data type for floating point tensors.
        learnable_center_freqs : bool, default=False
            If True, center frequencies become learnable parameters.
        learnable_shape_ratio : bool, default=False
            If True, shape ratio becomes a learnable parameter.
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f" device={self.device}{learnable_str})"
        )
    ...
    def plot_scalogram(
        ...
        return plot_tf_plane(
            ax=ax,
            freqs=self._center_freqs.detach().cpu().numpy(),
