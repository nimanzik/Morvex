...
class TaperConfig(BaseModel, frozen=True, extra="forbid"):
    ...
    def build_window(self) -> torch.Tensor:
        """Build a tapering window from this validated configuration."""
        try:
            window_fn = getattr(torch, f"{self.window_type}_window")
        except AttributeError as e:
            raise ValueError(f"Unsupported window type: {self.window_type}") from e

        max_half_lens = [self.n_samples // 2]

        if self.max_percentage is not None:
            max_half_lens.append(int(self.n_samples * self.max_percentage))

        if self.max_fade_len is not None:
            max_half_lens.append(self.max_fade_len)

        fade_len = min(max_half_lens)
        if fade_len == 0:
            return torch.ones(self.n_samples)

        window_kwargs: dict[str, float | bool] = {"periodic": False}
        if self.window_type == "kaiser":
            window_kwargs["beta"] = self.kaiser_beta

        fade_in_out = window_fn(2 * fade_len + 1, **window_kwargs)

        if self.side == "left":
            return torch.cat(
                [fade_in_out[:fade_len], torch.ones(self.n_samples - fade_len)]
            )
        if self.side == "right":
            return torch.cat(
                [torch.ones(self.n_samples - fade_len), fade_in_out[-fade_len:]]
            )
        return torch.cat(
            [
                fade_in_out[:fade_len],
                torch.ones(self.n_samples - 2 * fade_len),
                fade_in_out[-fade_len:],
            ]
        )
...
class Taper(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.max_percentage = cfg.max_percentage
        self.max_fade_len = cfg.max_fade_len
        self.side = cfg.side
        self.kaiser_beta = cfg.kaiser_beta
        self.register_buffer(name="weight", tensor=cfg.build_window())
